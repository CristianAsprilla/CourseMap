...
def main():
    ...
    # Expects the raw API JSON as 'result'.
    # """
    cleaned_docs = []
    for content in result.get("result", {}).get("contents", []):
        cleaned_docs.append(clean_document(content))

    # Output clean JSON
    json.dump(cleaned_docs, sys.stdout, indent=2, ensure_ascii=False)
...
def extract_pdf_to_json(pdf_bytes: bytes) -> dict[str, Any]:  # pyright: ignore[reportExplicitAny]
    """Analyze a PDF provided as bytes and return the raw result JSON."""
    ...
    settings = Settings(
        ...
        subscription_key=subscription_key,
        analyzer_id=analyzer_id,
        file_location="",  # not used for bytes flow
    )
    ...
    response = requests.post(
        url=client._get_analyze_url(client._endpoint, client._api_version, analyzer_id),
        headers=headers,
        data=pdf_bytes,
    )
    response.raise_for_status()
    return client.poll_result(response, timeout_seconds=60 * 60, polling_interval_seconds=1)
